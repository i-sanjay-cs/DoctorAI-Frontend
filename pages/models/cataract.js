...
const CataractDetection = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:5000/predict-cataract", {
        method: "POST",
        body: formData,
      });
...
const generateResponse = await cohere.generate({
    prompt: `Hello, I am MediMate. Generate advice for managing cataracts. Provide guidance on lifestyle changes, dietary recommendations, and preventive measures to alleviate symptoms and improve eye health. Additionally, suggest consulting a doctor for personalized evaluation and treatment options.
    `,
  });
      ...
      if (result.predicted_class.toLowerCase() === 'normal') {
        text = "That's great! You are fine. If you need more information, you can consult a doctor.";
      }
